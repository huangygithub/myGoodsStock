...
public class JDJumpTaskCmd implements Command<Void> {
	...
	public Void execute(CommandContext commandContext) {
		...
		while (localIterator.hasNext()) {
			...
			// 触发任务监听
			taskEntity.fireEvent(TaskListener.EVENTNAME_CREATE);
			// 删除任务的原因
			Context.getCommandContext().getTaskEntityManager().deleteTask(taskEntity, "completed1", false);
		}
